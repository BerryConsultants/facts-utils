#
# Example script for generating an AIPF TTE external data file
#
...
#
RunEndpointModel = function(n.pats.per.arm, groups, visits, n.arms.per.group, visit.params, resp.params, 
                            pat.visit.func, pat.resp.func)
{
  browser()
  ...
  # Get patient final response
  pat.resp = pat.resp.func(patient.id, group.allocation, treat.allocation, resp.params)
  ...
  # final visit
  visit.label = paste("Visit", sep="", toString(length(visits)))                     
  patient.responses[ ,visit.label] = pat.resp
  ...
  if (length(visits) > 1) {
    for (v in 1:length(visits)) {
      ...
      responses = pat.visit.func(pat.resp, resp.params, visits[v])
...
#
resp.visit.fnc <- function(pat.ids, pat.group, pat.treat, pat.res, cov.params){
...
#
resp.rate.fnc <- function(pat.ids, grp, trt, resp.params){
  resp <- rep(0, length(pat.ids))
  

  for (g in min(grp):max(grp)) {
    # gix - indexes of subjects in group g
    gix <- which(grp == i)
    if (length(gix > 0)) {
      for (t in min(trt):max(trt)) {
        # tix - indexes of sibjects in group g that are have treatment t
        tix <- which(trt[gix] == t)
        if (length(tix) > 0) {
          resp[gix[ix]] <- rbinom(length(gix[tix]), resp.params[g,t])
        }
      }
    }
  }
...
# "resp-cts.dat", groups.3, visits, 2, cov.param.expl, resp.params.expl, 100)
#
# gen.responders("C:/FACTS/FACTS 3.3.24 Test/FACTS 3.3.24 AIPF Cts Test Script Results", 
# "resp-cts-base-cfb.dat", groups.3, visits, 2, cov.param.expl, resp.params.expl, 100)
#
# gen.responders("C:/FACTS/FACTS 3.3.24 Test/FACTS 3.3.24 AIPF Cts Test Script Results", 
# "resp-cts-base-raw.dat", groups.3, visits, 2, cov.param.expl, resp.params.expl, 100)

# 
# Genresponder Cancer example 
#
cancer.example <- function(){
  can.groups <- c(1,2,3)
  can.visits <- c(0.33, 0.66, 1)
  # row per group - Pr(control responder), Pr(Treatment responder)
  can.cov.param <- rbind(c(0.2, 0.6), c(0.1, 0.3), c(0, 0.1))

  # row per group - mean times to event
  can.resp.params <- data.frame(nonrpndr.mean = 0,
                                nonrpndr.sd = 5, 
                                rpndr.mean = 4,
                                rpndr.sd = 5)
